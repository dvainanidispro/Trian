...
const intervalInMinutes = Number(process.env.ORDERRESENDINTERVAL ?? 15);
...
async function processQueue() {
    ...
    for (const entry of pendingEntries) {
        ...
        try {
            ...
            const wasSuccessful = softoneResponse.success && softoneResponse.data?.length > 0;
            const newStatus = wasSuccessful
                ? 'completed'
    ...
    sendOrderErrorEmail(failedOrders);
    isProcessing = false;
}
