...
let validateFirebaseToken = async (req, res, next) => {
    ...
    try{
        ...
        // χωρίς token στην παραγγελία, είναι req.headers.authorization=='Bearer undefined' διότι το header έχει ρυθμιστεί να στέλνεται πάντα. 
    ...
    if (!customer) {    // customer not found (token invalid/missing)
        console.error(`Unauthorized request on request route: ${req.originalUrl}`);         // req.get('Referrer') is always only the origin
